...
# GET REQUEST METHODS
    ...
    def get_bookings(payload):
        try:
            ...
            booking = {str(b.uuid): {**b.booking(),
                                     **b.guest.booking(),
                                     **b.room.booking(),
                                     **b.room.roomtype.booking()} for b in bookings}
            return jsonify({
                "success": True,
                "bookings": booking
            })
    ...
    def get_guests_by_uuid(payload, guest_uuid):
        ...
        try:
            ...
            if len(bookings) == 0:
                ...
                guest['bookings'] = {
                    str(b.uuid): {**b.guest_view(), **b.room.booking()} for b in bookings}
            return jsonify({
                "success": True,
                "guest": guest
            })
    ...
    def get_rooms(payload):
        try:
            rooms = {
                r.id: {
                    **r.booking(),
                    **r.roomtype.long_no_id(),
                    "dates_booked": booked_dates(r)} for r in Room.query.all()}
